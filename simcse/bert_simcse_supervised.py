...
parser.add_argument("--batch_size_eval", type=int, default=10)
...
parser.add_argument("--train_file", type=str, default="data/simclue/train_rank.json")
parser.add_argument("--dev_file", type=str, default="data/simclue/test_public.json")
parser.add_argument("--test_file", type=str, default="data/simclue/test_public.json")
...
class TrainDataSet(Dataset):
    ...
    def load_data(self, data_file):
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                ...
                all_data.append([line_json["query"], line_json["title"], line_json["neg_title"]])
    ...
    def __getitem__(self, idx):
        return tokenizer([self.data[idx]], max_length=args.max_length,
...
class TestDataSet(Dataset):
    ...
    def load_data(self, data_file):
        ...
        with open(data_file, 'r', encoding='utf-8') as f:
            ...
            for line in lines:
                line_json = json.loads(line)
                all_data.append([line_json["sentence1"], line_json["sentence2"], int(line_json["label"])])
        return all_data
...
def train_loop(train_dataloader, test_dataloader, model, optimizer, lr_scheduler, epoch, total_loss, best_acc):
    ...
    for step, inputs in enumerate(train_dataloader, start=1):
        input_ids = inputs['input_ids'].view(len(inputs['input_ids']) * 3, -1).to(args.device)
        attention_mask = inputs['attention_mask'].view(len(inputs['attention_mask']) * 3, -1).to(args.device)
        token_type_ids = inputs['token_type_ids'].view(len(inputs['token_type_ids']) * 3, -1).to(args.device)
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.AdamW(bertSimCSEModel.parameters(), lr=args.lr)
    # 动态调整学习率
    ...
    print("Done!")
    print('Start testing...')
    bertSimCSEModel.load_state_dict(
        torch.load('./checkpoints/bert_simcse_supervised/model_weights.bin', map_location=args.device))
    acc = test_loop(test_dataloader, bertSimCSEModel)
    print(acc)
